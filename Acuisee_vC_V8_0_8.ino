...
// 10.28.2019     rjw 8.0.6 cleaned up code, adjusted pusher block travel distance to new shortened model
// 10.20.2019     rjw 8.0.7 added return of 'n' full fwd even if steps to move is 0 . . . signal fully extended
// 11.06.2019     rjw 8.0.8 removed delay after turning on R or L pushbutton light
//
// HISTORY ******************************************************************************************************
//
// **************************************************************************************************************
...
const long    maxSteps           = 16000;         // steps for full travel
...
void setup() {
  ...
  // OUTPUTS
  pinMode(leftPBled,  OUTPUT);            // PB internal led
  pinMode(rightPBled, OUTPUT);            // PB internal led
  pinMode(runLED,     OUTPUT);            // led on arduino uno board, usually D13
  pinMode(DIR_PIN,    OUTPUT);            // stepper
  pinMode(STEP_PIN,   OUTPUT);            // stepper


  // Set Initial Values for LEDs
  digitalWrite(leftPBled,  HIGH);         // for alternating pattern in setup flash code
  digitalWrite(rightPBled, LOW);          // for alternating pattern in setup flash code
  digitalWrite(runLED,     LOW);

  // Invert to state flags to force a read first time through
  ...
  Serial.print(F("step per ml   = ")); Serial.println(steps_per_ml);
  Serial.println(F("-----------------------------------------"));
  ...
  if ((millis() - lastDebounceTime) > debounceDelay) {
    ...
    if (digitalRead(leftPB) != leftPBstate) {
      ...
      if (leftPBstate == LOW) {
        ...
        // delay(2000); // leave led on for a couple of seconds
      } else {
        Serial.write("b\n"); Bluetooth.print("b\n");
        digitalWrite(leftPBled,  LOW);
      }
...
char processCommand(char ch) {
  switch (ch) {
    case 'S':     // Stats
      Serial.print("processCommand : "); Serial.println(ch);
      stats();
      return 0;
      break;
    case 'R':     // RESET
    ...
    case '?':
      ...
      break;
    default:
      Serial.print("DEFAULT processCommand : "); Serial.println(ch);
...
// checkSteps ******************************************************************************************************
long checkSteps(long checkVal) {
  if (checkVal > 0) {
    return (checkVal); // if positive we're moving towards LS stop, no need to check steps
  } else {
    long stepsRemaining = maxSteps - stepCounter;
    if (stepsRemaining > (abs(checkVal))) {
    // Serial.print("StepsToTravel = "); Serial.println(checkVal);  
    return (checkVal);        // return value passed
    } else {
      // Serial.print("StepsToTravel = "); Serial.println(-stepsRemaining);
      if (-stepsRemaining == 0) {
        Serial.write("n\n"); Bluetooth.print("n\n");
      }
